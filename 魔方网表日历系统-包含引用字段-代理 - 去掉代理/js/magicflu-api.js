...
class MagicFluApi {
    constructor() {
        ...
        this._savedCredsKey = 'mf_saved_credentials';
    ...
    /**
     * 直接登录魔方网表（无需本地代理）
     * options: { remember: boolean, silent: boolean }
     */
    async login(username, password, options = { remember: true, silent: false }) {
        try {
            const baseUrl = this.servers[this.currentServerType];
            const url = `${baseUrl}/magicflu/jwt`;

            const body = new URLSearchParams({ j_username: username, j_password: password }).toString();
            ...
            // 魔方登录接口返回结构可能为 { status: 1, token, username, nickname, id }
            if ((data && data.status === 1) || data.success) {
                const token = data.token || data.accessToken || data.jwt || '';
                this.token = token;
                this.tokenExpiry = Date.now() + 30 * 60 * 1000; // 30 分钟
                this.userInfo = {
                    username: data.username || data.user || (data.userInfo && data.userInfo.username),
                    nickname: data.nickname || (data.userInfo && data.userInfo.nickname),
                    id: data.id || (data.userInfo && data.userInfo.id)
                };
                ...
                // 保存凭证以便自动刷新（使用 base64 简单加密，注意：生产环境请使用更安全的方案）
                if (options.remember) {
                    try {
                        this.saveCredentials(username, password);
                    } catch (e) {
                        console.warn('无法保存凭证', e);
                    }
                }
    ...
    clearStoredAuth() {
        ...
        localStorage.removeItem(this._savedCredsKey);
        this.token = null;
        this.tokenExpiry = null;
        this.userInfo = null;
    ...
    // 简单的凭证保存（base64 编码，非加密，生产环境请使用更安全的方法）
    saveCredentials(username, password) {
        try {
            const payload = btoa(JSON.stringify({ username, password }));
            localStorage.setItem(this._savedCredsKey, payload);
        } catch (e) {
            console.warn('保存凭证失败', e);
        }
    }

    getSavedCredentials() {
        try {
            const raw = localStorage.getItem(this._savedCredsKey);
            if (!raw) return null;
    ...
    /**
     * 设置自动刷新：在 token 到期前 5 分钟尝试用保存的凭证重新登录
     */
    setupAutoRefresh() {
        if (this._tokenRefreshTimeout) {
            clearTimeout(this._tokenRefreshTimeout);
        ...
        const now = Date.now();
        const msBeforeExpiry = this.tokenExpiry - now - 5 * 60 * 1000;
        const delay = Math.max(0, msBeforeExpiry);
    ...
    async attemptRefresh() {
        ...
        if (!creds || !creds.username || !creds.password) {
            // 无法自动刷新
            this.clearStoredAuth();
            console.warn('自动刷新失败：没有找到保存的凭证');
            return;
        }
        // 尝试静默登录刷新 token
        const res = await this.login(creds.username, creds.password, { remember: true, silent: true });
        if (!res.success) {
            console.warn('自动刷新登录失败', res.message || res);
            this.clearStoredAuth();
        }
    }
     ...
     */
    buildBaseUrl(path) {
        const base = this.servers[this.currentServerType];
        return `${base}${path}`;
    }
    ...
    async getWorkTasks(start = 0, limit = -1, bq = '') {
        try {
            ...
            let url = this.buildBaseUrl(`/magicflu/service/s/jsonv2/${this.workTaskForm.appId}/forms/${this.workTaskForm.formId}/records/entry`);
    ...
    async addWorkTask(taskData) {
        try {
            ...
            const url = this.buildBaseUrl(`/magicflu/service/s/jsonv2/${this.workTaskForm.appId}/forms/${this.workTaskForm.formId}/records`);
    ...
    async getPersonnel(start = 0, limit = -1, bq = '') {
        try {
            ...
            let url = this.buildBaseUrl(`/magicflu/service/s/jsonv2/${this.personnelForm.appId}/forms/${this.personnelForm.formId}/records/entry`);
            const params = new URLSearchParams({
                start: start.toString(),
                limit: limit.toString()
            });
